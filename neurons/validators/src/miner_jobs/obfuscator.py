...
def random_code_block():
    ...
    if random_int % 3 == 0:
        print("Generating random for block")
        return random_for_block()
    elif random_int % 3 == 1:
        print("Generating random if block")
        return random_if_generator()
    else:
        print("Skipping generating random code block")
        return None
...
class CodeObfuscator(ast.NodeTransformer):
    ...
    def visit_Name(self, node: Name):
        ...
        if not node.id.startswith('__'):
            if node.id not in self.name_mapping:
                self.name_mapping[node.id] = self.generate_random_name()
            node.id = self.name_mapping[node.id]
...
class FunctionObfuscator(ast.NodeTransformer):
    ...
    def visit_FunctionDef(self, node: FunctionDef):
        ...
        for arg in node.args.args:
            args_mapping[arg.arg] = generate_random_name(length=10)
